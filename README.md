...
| --------------- | -------------- | -------------------------------- |
| **MostPopular**           | Эвристичеаская модель, на основе статистики берем N самых популярных треков     | Статистика формируется на основе явной обратной связи — лайков. Поскольку тренды и популярные треки меняются динамично, временное окно для определения актуальных хитов должно быть достаточно коротким, чтобы отражать свежие предпочтения пользователей.       |
| **NewItemsLastNDays**           | Эвристичеаская модель, top-N популярных песен, появившиеся недавно   | Похоже на модель популярных песен, но берет только новые песни. В топ прослушаных могут попадать к примеру песня Шуфутинского 3го сентября       |
