...
|------------------|--------|--------------|---------|
| **Recall@K** | При музыкальных рекомендациях важно предложить пользователю как можно больше релевантных треков. Даже если среди рекомендаций окажутся «ошибки», пользователь всё равно сможет найти что-то подходящее.| Долю релевантных треков, найденных в топ-K рекомендаций | $$Recall@K = \frac{|Rel \cap Rec_K|}{|Rel|}$$ |
| **nDCG@K** | Когда пользователь открывает плейлист «Тебе понравится», критично удержать его внимание с первых позиций. Если первые треки не подходят, человек редко будет искать «бриллианты» внизу списка. Метрика учитывает порядок рекомендаций и даёт больший вес верхним позициям. Так можно оценить качество ранжирования, а не только факт попадания релевантных треков в топ-K. | Качество ранжирования с учётом позиций | $$DCG@K = \sum_{i=1}^{K} \frac{rel_i}{\log_2(i+1)}$$ <br> $$IDCG@K = \sum_{i=1}^{|Rel_K|} \frac{1}{\log_2(i+1)}$$ <br> $$nDCG@K = \frac{DCG@K}{IDCG@K}$$ |
| **Coverage** | Рекомендации должны быть разнообразными, а не состоять только из самых популярных треков. Эта метрика показывает, какую долю всей музыкальной коллекции система вообще использует в рекомендациях.  Чем выше coverage, тем меньше «туннельного видения» и тем более разнообразной и персональной может быть рекомендация. | Долю каталога, который система использует в рекомендациях | $$Coverage = \frac{|\bigcup\_{u \in Users} Rec_K(u)|}{|Items|}$$ |
| **Cosine Similarity** | Это дополнительная метрика для анализа.  Так как датасет статичный и невозможно проверить, понравился бы трек пользователю в реальности, приходится оценивать качество косвенно. Она показывает, насколько эмбеддинги рекомендованных мной треков похожи на эмбеддинги тех, что пользователь действительно слушал (или тех, что рекомендовала система Яндекса).  Это помогает понять, движется ли моя модель в правильном направлении, даже если прямой обратной связи нет. | Близость эмбеддингов рекомендованных и прослушанных треков | $$cosine\_similarity(A, B) = \frac{A \cdot B}{\|A\| \|B\|}$$ |
