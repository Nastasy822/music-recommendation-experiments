...
#### Очиска данных 
Применим необходимую предобработку для очистки и подготовки данных. Подробные рассуждения о шагах и мотивации предобработки приведены в блокноте notebooks/data_EDA.
...
| **4. Удаление редких треков**                   | Только Train                   | Треки с очень малым числом прослушиваний убираются как шумовые.                                    |
| **5. Удаление треков, отсутствующих в системе** | Только Train (Test — оставить) | Из Train удаляем треки без embedding и метаданных; считаем метрику отдельно с ними и без них.      |
| **6. Удаление “редких” пользователей**          | Только Train                   | Пользователи с малым числом событий ухудшают обучение модели.                                      |
| **7. Mapping одинаковых песен по embeddings**          | Везде                   | Один и тот же трек может быть записан несколько раз под разными item_id                                      |
