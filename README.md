...
- [Что предсказываем и как будем оценивать](#что-предсказываем-и-как-будем-оценивать-метрики-x-сы-y-ки-data-split)
- [Работа с данными](#📥-работа-с-данными)
- [Схема системы](#схема-системы)
- [Точность моделей](#точность-моделей)
- [Выводы](#выводы)
- [Копилка идей и планы на будущее](#копилка-и-идей-и-планы-на-будущее)
...
> Для обучения модели ранжирования дополнительно происходит деление по пользователям, чтобы модель могла эффективно ранжировать данные даже для новых пользователей, чьи предпочтения ранее не встречались в обучающей выборке.
...
flowchart LR
    ...
    subgraph RETRIEVAL ["Retrieval 🔍"]
        direction TB
        PopAll["MostPopular<br/>⚙️ 50 items, last 5 days"]
        PopUser["NewItemsLastNDays<br/>⚙️ 50 items, last 5 days"]
        SIM["KMeansEmbedding<br/>⚙️ 100 items"]
        ITEMKNN["ItemKNN<br/>⚙️ 200 items<br/>weights: exp(time_decay)"]
        GRAPH["RandomWalkWithRestart<br/>⚙️ 100 items<br/>weights: log(count_listen)<br/>user-item, item-artist, item-album"]
        ALS["ALS<br/>⚙️ 200 items<br/>weights: log(count_listen)"]
        BM25["BM25<br/>⚙️ 200 items<br/>weights: log(count_listen)"]
    end
...
Таблица ниже сравнивает качество различных рекомендательных моделей по четырём ключевым метрикам: Recall@10, NDCG@10, Coverage@10 и Cosine Similarity@10. Значения указаны в %



| Модель                                 | Recall@10 | NDCG@10 | Coverage@10 | CosineSim@10 | Recall@100 | NDCG@100 | Coverage@100 | CosineSim@100 |
|----------------------------------------|-----------|---------|--------------|---------------|-------------|-----------|----------------|----------------|
| **Baseline**                           | **3.700** | **7.810** | **2.570**    | **-**     | **12.970**  | **9.340** | **9.990**      | **-**       |
| MostPopular                            | 1.530     | 2.600   | 0.000        | 46.070        | 4.568       | 3.104     | 0.0296         | 41.597         |
| NewItemsLastNDays                      | 0.660     | 1.520   | 0.000        | 45.850        | 0.908       | 1.053     | 0.0296         | 36.889         |
| ALS                                    | 1.890     | 4.140   | 0.180        | 61.280        | 8.486       | 5.647     | 0.8998         | 59.707         |
| BM25                                   | 3.360     | 6.330   | 9.460        | 64.190        | 14.441      | 9.165     | 37.889         | 63.222         |
| ItemKNN                                | 4.307     | 8.100   | 2.620        | 63.060        | 12.305      | 9.337     | 11.343         | 59.574         |
| BPR                                    | 0.960     | 2.430   | 4.460        | 64.350        | 6.792       | 4.074     | 17.315         | 62.826         |
| RandomWalkWithRestart                  | 5.600     | 10.580  | 5.650        | 64.460        | 20.968      | 14.310    | 21.780         | 59.054         |
| KMeansEmbedding                        | 2.660     | 4.350   | 6.520        | 69.226        | 5.508       | 4.472     | 29.146         | 68.857         |
| **HybridModel**                        | **9.858** | **16.500** | **6.759**    | **68.236**    | **24.838**  | **19.053** | **24.985**     | **64.023**     |
| HybridModel + Filt.                    | 4.157     | 7.893   | 6.886        | 66.000        | 15.958      | 10.862     | 24.462         | 63.208         |
| HybridModel + Filt. + Diver.           | 3.720     | 7.411   | 6.711        | 64.859        | 12.366      | 9.080     | 22.130         | 59.984         |
| **HybridModel + Filt. + Diver. + EG**  | **3.349** | **6.902** | **6.395**    | **62.591**    | **11.752**   | **8.774** | **21.957**      | **58.522**     |




Графики показывают, как разные рекомендательные модели выступают по ключевым метрикам качества: Recall@10, NDCG@10, Coverage@10 и Cosine Similarity@10. Каждая точка — результат конкретного алгоритма. Красная горизонтальная линия — это бенчмарк (лучшее эталонное значение), служащее ориентиром: модели выше линии превосходят базовый уровень, модели ниже — уступают ему.
...
Разработана каскадная система рекомендаций, состоящая из нескольких малых retrieval-моделей, выход которых проходит последовательное ранжирование и постобработку результатов (подробнее — на представленной схеме). Переход от экспериментов в Jupyter Notebook к DVC-пайплайну позволил существенно повысить воспроизводимость и гибкость экспериментов, а также оптимизировать процесс разработки и отладки.

Основными ориентирами при построении системы стали метрики по топ-10: NDCG, Recall, Coverage, а также дополнительная метрика — косинусное сходство. Было достигнуто значительное превышение baseline: NDCG вырос с 7.8 до 16.5, Recall — с 3.7 до 9.85, Coverage — с 2.6 до 6.75. Все измерения проводились в одинаковых условиях, что обеспечивает корректное сравнение. В реальных условиях, при необходимости фильтрации, учета exploration и других ограничений, точность несколько снизилась, однако косинусное сходство осталось на высоком уровне, что свидетельствует о сохранении качества рекомендаций с точки зрения предпочтений пользователя.

Точность рекомендаций, безусловно, ниже, чем в классических задачах компьютерного зрения (где норма — 90%+), однако для музыкального каталога с сотнями тысяч треков, где большинство треков слушаются лишь 1–2 раза, а поведение пользователей крайне разнообразно, такие значения являются типичными и объяснимыми. Система продолжает развиваться, и в планах — дальнейшие улучшения архитектуры и алгоритмов для повышения качества рекомендаций.



## Копилка идей и планы на будущее

### Предобработка данных
- Добавить статус "skip" для корректного учета пропущенных треков.
- Проверить наличие и частоту повторных прослушиваний.
- Ввести статус "добавил в плейлист" для анализа переходов между органическими и неорганическими прослушиваниями.
- Исследовать причины появления повторов в один и тот же timestamp.
- Разделить данные на две выборки: одна — очищенная от выбросов для ALS, вторая — полная, включающая короткие и длинные треки для анализа и сравнения.

### Профиль пользователя
- Повысить качество формирования векторов пользователя для более точного поиска похожих песен.
- Оптимизировать процесс кластеризации пользователей для улучшения персонализации.

### Фичи
- Учесть сезонность в моделях для повышения релевантности рекомендаций.
- Внедрить метрику Exploration Ratio для контроля баланса между новыми и знакомыми рекомендациями.

### Retrival-модели
- Протестировать SASRec для последовательных рекомендаций.
- Заменить граф на GNN для улучшения качества retrieval.
- Экспериментировать с различными весами матрицы для моделей.

### Ранжирование
- Внедрить window-based подход при подготовке датасета: использовать не готовый train_df, а исторические срезы.
- Протестировать альтернативные целевые переменные для моделей.

### Инфраструктура
- Перейти от Faiss к векторной базе данных Qdrant для масштабируемости.
- Организовать логирование метрик на всех этапах пайплайна.
- Добавить Optuna для автоматизированного подбора гиперпараметров.



## 🛠️ Local Development Setup

Этот проект использует Dev Containers, Docker и DVC. Ниже приведена инструкция по запуску окружения, подготовке данных и выполнению эксперимента.
...
###  Запустить эксперимент с помощью DVC

Убедитесь, что вы находитесь внутри dev-контейнера и в корне проекта, затем выполните:
```bash
dvc exp run
```
DVC сам подтянет зависимости, выполнит пайплайн и сохранит результаты эксперимента.
